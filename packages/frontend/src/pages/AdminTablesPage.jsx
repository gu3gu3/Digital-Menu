    ...
    <div className="p-6">
      ...
      {loading && mesas.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {mesas.map((mesa) => (
            <div key={mesa.id} className="bg-white rounded-lg shadow hover:shadow-md transition-shadow">
              <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center">
                    <div className="flex-shrink-0">
                      <div className="w-10 h-10 bg-gradient-to-r from-primary-500 to-secondary-500 rounded-lg flex items-center justify-center">
                        <span className="text-white font-bold">{mesa.numero}</span>
                      </div>
                    </div>
                    <div className="ml-4">
                      ...
                      {mesa.nombre && (
                        <p className="text-sm text-gray-500">{mesa.nombre}</p>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center space-x-1">
                    {mesa.ordenesActivas > 0 && (
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                        {mesa.ordenesActivas} orden{mesa.ordenesActivas !== 1 ? 'es' : ''}
                      </span>
                    )}
                    {!mesa.activa && (
                ...
                {mesa.descripcion && (
                  <p className="text-sm text-gray-600 mb-4">{mesa.descripcion}</p>
                )}

                <div className="flex items-center justify-between text-sm text-gray-500 mb-4">
                  <span>Capacidad: {mesa.capacidad} personas</span>
                  <span className={`font-medium ${mesa.activa ? 'text-green-600' : 'text-gray-400'}`}>
